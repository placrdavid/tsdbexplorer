...
#	@conn.prepare("get_basic_schedule_uuid_for_activation_msg_plan", get_basic_schedule_uuid_for_activation_msg_sql)
	...
	#   # get a schedule's origin name TODO - BETTER WAY OF DOING THIS?
	...
	#   # get a schedule's destination name TODO - BETTER WAY OF DOING THIS?
...
#2 CHECKS TO DO HERE
#1. FIND THE TRAINS THAT RUNS TODAY
#2. FOR THESE SORT BY STP_INDICATOR (AND TAKE, LOWEST?)
	puts '------------------------------------'
	puts 'n_matching_uuids = '+n_matching_uuids.to_s
	# GET THE A (THE?) RESULT THAT RUNS TODAY
	#matching_services_right_day = Array.new
	matching_services_right_day = Hash.new
	# TODO autoget day index
	# get date of departure
	# find day associated with day of departure
	puts "origin_dep_timestamp.wday = "+origin_dep_timestamp.wday.to_s
	# transform day to dayindex
	day_index = 'runs_su'
	case origin_dep_timestamp.wday
	when 0
		day_index = 'runs_su'
	when 1
		day_index = 'runs_mo'
	...
	puts "day_index = "+day_index.to_s	
	matching_uuid_res.each do |matching_uuid|
		p matching_uuid
		if matching_uuid[day_index] == 't'
			stp_indicator = matching_uuid['stp_indicator']
#			matching_services_right_day.push(matching_uuid)
# => 		TODO check for multiple matches of stp_indicator
			unless matching_services_right_day[stp_indicator].nil?
				puts 'activation msg matches on two (near) identical schedules:'
				puts 'same train_uid '+train_uid.to_s+ ', both run on '+day_index+', and they have the same STP_indicator '+stp_indicator
				p matching_uuid
				p matching_services_right_day[stp_indicator]
				puts 'duplicates in schedules DB?'
			end
			matching_services_right_day[stp_indicator] = matching_uuid
			puts 'this service runs on the right day - result!'
		else 
			puts 'this service does not run on the right day!'			
		end
	end
	puts 'matching_services_right_day n = '+matching_services_right_day.size.to_s


	# if there are no schedules, report this
	if matching_services_right_day.size == 0
		puts Time.now.to_s+': PROBLEM PROCESSING 0001 MSG: no matching basic_schedule_uuid for schedule_start_date='+schedule_start_date+' train_service_code ='+train_service_code+' origin_dep_hhmm = '+origin_dep_hhmm+'' 
		puts Time.now.to_s+': SEARCHED basic_schedules usign train_uid='+train_uid.to_s+' and origin_depart_date ='+origin_depart_date.to_s+', for day of week '+day_index.to_s 
		p indiv_msg
		puts '-------'		
	else
		# if one (or more) get the schedule with the lowest stp alpha key
		matching_schedule = matching_services_right_day.min[1]
		basic_schedule_uuid = matching_schedule['uuid']
		atoc_code = matching_schedule['atoc_code']

		# get the destination name 
		destname =  matching_schedule['destin_name'].to_s

		# get the origin name 
		originname =  matching_schedule['origin_name'].to_s
		...
		puts Time.now.to_s+': 0001 msg - now tracking service with basic_schedule_uuid '+basic_schedule_uuid+''        unless @quiet                 
		...
		# insert new
		res = @conn.exec_prepared("store_live_msg_plan", [msg_type, basic_schedule_uuid, train_id, indiv_msg['body'].to_json, Time.new, Time.new]) 
		puts 'we matched the incoming  activation against a single schedule with STP indicator '+matching_schedule['stp_indicator'].to_s+'! DOUBLE BONUS'
	end
...
=begin	
	...
	if n_matching_uuids==1
		...
		# get the destination name 
		#puts "destination = "+ matching_uuid_res[0]['destin_name'].to_s
		#resdest = @conn.exec_prepared("get_schedules_destination_name_plan", [basic_schedule_uuid]) 
		#destname = resdest[0]['tps_description']
		#puts "destname = "+ destname.to_s
		destname =  matching_uuid_res[0]['destin_name'].to_s

		# get the origin name 
		#puts "origin = "+ matching_uuid_res[0]['origin_name'].to_s
		#resorigin = @conn.exec_prepared("get_schedules_origin_name_plan", [basic_schedule_uuid]) 
		#originname = resorigin[0]['tps_description']
		#puts "originname = "+ originname.to_s
		originname =  matching_uuid_res[0]['origin_name'].to_s
		...
		puts Time.now.to_s+': PROBLEM PROCESSING 0001 MSG: no matching basic_schedule_uuid for schedule_start_date='+schedule_start_date+' train_service_code ='+train_service_code+' origin_dep_hhmm = '+origin_dep_hhmm+'' 
		puts Time.now.to_s+': SEARCHED basic_schedules usign train_uid='+train_uid.to_s+' and origin_depart_date ='+origin_depart_date.to_s+'' 
		p indiv_msg
		puts '-------'
	else
		# TODO these can be distinguished by the origin tiploc in timetables??
		puts Time.now.to_s+': PROBLEM PROCESSING 0001 MSG: multiple matching basic_schedule_uuid for schedule_start_date='+schedule_start_date+' train_service_code ='+train_service_code+' origin_dep_hhmm = '+origin_dep_hhmm+'' 
		puts Time.now.to_s+': SEARCHED basic_schedules usign train_uid='+train_uid.to_s+' and origin_depart_date ='+origin_depart_date.to_s+'' 
		p indiv_msg
		puts '-------'
	end
	...
	def receive_msg msg
		...
		if msg.command == "CONNECTED"
			...
			msg_list.each do |indiv_msg|
				...
				unless  msg_type == '0001'     
					if tracked_train.nil?
						puts Time.now.to_s+": the service associated with the following msg "+train_id+" has not been activated, so we can't xref with timetables"    unless @quiet          
						p indiv_msg	unless @quiet         
					end
				end
				...
				if msg_type == '0002'
					@stats_n_0002_msgs+=1
					process_cancellation_msg(indiv_msg, tracked_train)   unless tracked_train.nil?
				end
				# Message 3 – 0003 – Train Movement
				if msg_type == '0003'
					@stats_n_0003_msgs+=1
					#if tracked_train.nil?
						unless tracked_train.nil?
							puts 'tracked : process the train movement msg'
							opblock_0003 = proc {
							process_trainmovement_msg(indiv_msg, tracked_train)
							}
							EventMachine.defer(opblock_0003)   
						else
							puts 'not tracked: cant process the train movement msg'							
						end
					#end
				end
				...
				if msg_type == '0004'
					@stats_n_0004_msgs+=1
					puts "got a 0004 msgs"
					process_unidentifiedtrain_msg(indiv_msg, tracked_train)     unless tracked_train.nil?
				end
				# Message 5 – 0005 – Train Reinstatement
				if msg_type == '0005'          
					@stats_n_0005_msgs+=1
					puts "got a 0005 msgs"
					process_trainreinstatement_msg(indiv_msg, tracked_train)     unless tracked_train.nil?
				end
				# Message 6 – 0006 – Train Change of Origin
				if msg_type == '0006'
					@stats_n_0006_msgs+=1
					puts "got a 0006 msgs"
					process_trainchangeoforigin_msg(indiv_msg, tracked_train)     unless tracked_train.nil?
				end
				# Message 7 – 0007 – Train Change of Identity
				if msg_type == '0007'
					@stats_n_0007_msgs+=1
					puts "got a 0007 msgs"
					process_trainchangeofidentify_msg(indiv_msg, tracked_train)     unless tracked_train.nil?
				end
				# Message 8 – 0008 – Train Change of Location
				if msg_type == '0008'
					@stats_n_0008_msgs+=1
					puts "got a 0008 msgs"
					process_trainchangeoflocation_msg(indiv_msg, tracked_train)     unless tracked_train.nil?
				end
